...
We saw before how you can load a model from a model specification file using
`mread()`. Don't worry for now what is in that file; we'll show you how to
create it in @sec-specification.
...
`.cpp` extension and think that _all_ the code is `C++` and format the code in
ways that aren't what you usually want. So using `.mod` (or `.txt`) can be
helpful just to keep your editor from doing too much.
...
you can make these changes at the time the model is loaded.
...
These are pre-coded models that can be sourced from witin the mrgsolve 
...
You can list out the models that are available from the library with a plain 
call to
...
Query the compartment list with `init()`. Using the model we loaded in the 
previous section
...
model in terms of named compartments (e.g. `A(2)` or `F1`) as well. 
...
Updates can be made through `update()`. For example, use
